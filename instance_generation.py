...
def generate_sports_schedule(filename, n_weeks, n_teams):

    weeks = list(range(1, n_weeks + 1))
    teams = list(range(1, n_teams + 1))

    mdl = gu.Model('schedule')
    
    # what teams play when
    x = {(t1, t2, w): mdl.addVar(vtype=gu.GRB.BINARY, name=f'x_{t1}_{t2}_{w}')
         for t1 in teams for t2 in teams for w in weeks}
    
    # revenue for each game
    r = {(t1, t2, w): np.random.uniform(low=0, high=10)
         for t1 in teams for t2 in teams for w in weeks}
    
    mdl.setObjective(
        sum(sum(sum(r[t1, t2, w]*x[t1, t2, w] for t1 in teams) for t2 in teams) for w in weeks),
        sense=gu.GRB.MAXIMIZE
    )
    
    # 1) each matchup at most once
    for t1 in teams:
        for t2 in teams:
            mdl.addConstr(sum(x[t1, t2, w] + x[t2, t1, w] for w in weeks) <= 1,
                          name=f'repeat_matchup_{t1}_{t2}')
            
    # 2) each team plays exactly once per week
    for t in teams:
        for w in weeks:
            mdl.addConstr(sum(x[a, t, w] for a in teams) + sum(x[t, h, w] for h in teams) <= 1,
                          name=f'{t}_plays_week_{w}')

    # 3) even home and away
    for t in teams:
        mdl.addConstr(sum(sum(x[a, t, w] for w in weeks) for a in teams) ==
                      sum(sum(x[t, h, w] for w in weeks) for h in teams),
                      name=f'even_home_away_{t}')
        
    # 4) team can't play self
    for t in teams:
        for w in weeks:
            mdl.addConstr(x[t, t, w] == 0, name=f'identity_{t}_{w}')
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        ...
        choices=['setcover', 'cauctions', 'facilities', 'indset', 'schedules'],
    )
    ...
    if args.problem == 'setcover':
        ...
        # train instances
        n = 1000
        lp_dir = f'data/instances/schedules/train_{number_of_weeks}_{number_of_teams}_{ratio}'
        print(f"{n} instances in {lp_dir}")
        os.makedirs(lp_dir)
        filenames.extend([os.path.join(lp_dir, f'instance_{i+1}.lp') for i in range(n)])
        nweekss.extend([number_of_weeks] * n)
        nteamss.extend([number_of_teams] * n)
        ratios.extend([ratio] * n)

        # validation instances
        n = 200
        lp_dir = f'data/instances/schedules/valid_{number_of_weeks}_{number_of_teams}_{ratio}'
        print(f"{n} instances in {lp_dir}")
        os.makedirs(lp_dir)
        filenames.extend([os.path.join(lp_dir, f'instance_{i+1}.lp') for i in range(n)])
        nweekss.extend([number_of_weeks] * n)
        ...
        number_of_weeks = 8
        number_of_teams = 20
        lp_dir = f'data/instances/schedules/transfer_{number_of_weeks}_{number_of_teams}_{ratio}'
        print(f"{n} instances in {lp_dir}")
        os.makedirs(lp_dir)
        filenames.extend([os.path.join(lp_dir, f'instance_{i+1}.lp') for i in range(n)])
        nweekss.extend([number_of_weeks] * n)
        ...
        number_of_weeks = 8
        number_of_teams = 20
        lp_dir = f'data/instances/schedules/test_{number_of_weeks}_{number_of_teams}_{ratio}'
        print(f"{n} instances in {lp_dir}")
        os.makedirs(lp_dir)
        filenames.extend([os.path.join(lp_dir, f'instance_{i+1}.lp') for i in range(n)])
        nweekss.extend([number_of_weeks] * n)
        ...
        # actually generate the instances
        for filename, ncs, nfs, r in zip(filenames, nweekss, nteamss, ratios):
            print(f"  generating file {filename} ...")
            generate_sports_schedule(filename, n_weeks=ncs, n_teams=nfs)
