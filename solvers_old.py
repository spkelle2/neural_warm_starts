...
class BaseSolver(abc.ABC):
    ...
    def __init__(self, mip_pth: str, sampler: sampling.BaseSampler = None):
...
class NeuralDivingSolver(BaseSolver):
    ...
    def solve(self, unfixed_variables: int, n_samples: int) -> Dict[str, Any]:
        sub_mip, stats = predict_and_create_sub_mip(self._sampler, unfixed_variables, n_samples)
        status, sol_data, sub_mip_sol = gurobi_solve(
            sub_mip, self._solver_config.scip_solver_config, sol_data, timer)
        if self._solver_config.enable_restart:
            status, sol_data, _ = gurobi_solve(
...
class NeuralNSSolver(BaseSolver):
    ...
    def solve(
        ...
        if sampler_name not in sampling.SAMPLER_DICT:
            ...
            status, sol_data, incumbent_sol = gurobi_solve(
            ...
            _, sol_data, incumbent_sol = gurobi_solve(sub_mip,
        ...
        for s in range(self._solver_config.num_solve_steps):
            ...
            status, sol_data, improved_sol = gurobi_solve(
                sub_mip, self._solver_config.scip_solver_config, sol_data, timer,
                incumbent_sol)
